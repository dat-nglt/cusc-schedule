...
import { APIResponse } from "../utils/APIResponse"; // Đảm bảo APIResponse được import chính xác và hoạt động như mong đợi
...
export const getAllSubjectsController = async (req, res) => {
    try {
        ...
        return APIResponse(res, 500, null, error.message || "Đã xảy ra lỗi khi lấy danh sách môn học.");
    }
};
...
export const getSubjectByIdController = async (req, res) => {
    ...
    try {
        ...
        return APIResponse(res, 500, null, error.message || "Đã xảy ra lỗi khi lấy thông tin môn học.");
    }
};
...
export const createSubjectController = async (req, res) => {
    ...
    try {
        ...
        return APIResponse(res, 500, null, error.message || "Đã xảy ra lỗi khi tạo môn học.");
    }
};
...
export const updateSubjectController = async (req, res) => {
    ...
    try {
        ...
        return APIResponse(res, 200, subject, "Cập nhật thông tin môn học thành công.");
    } catch (error) {
        console.error(`Lỗi khi cập nhật môn học với ID ${id}:`, error);
        return APIResponse(res, 500, null, error.message || "Đã xảy ra lỗi khi cập nhật môn học.");
    }
};
...
export const deleteSubjectController = async (req, res) => {
    ...
    try {
        ...
        return APIResponse(res, 500, null, error.message || "Đã xảy ra lỗi khi xóa môn học.");
    }
};
...
export const getSubjectsBySemesterController = async (req, res) => {
    ...
    try {
        ...
        if (!subjects || subjects.length === 0) {
            return APIResponse(res, 404, null, "Không tìm thấy môn học nào cho học kỳ này.");
        }
        return APIResponse(res, 200, subjects, "Lấy danh sách môn học theo học kỳ thành công.");
    } catch (error) {
        console.error(`Lỗi khi lấy môn học cho học kỳ ${semesterId}:`, error);
        return APIResponse(res, 500, null, error.message || "Đã xảy ra lỗi khi lấy danh sách môn học theo học kỳ.");
    }
};
...
export const importSubjectsFromJSONController = async (req, res) => {
    ...
    try {
        ...
        if (!subjects || !Array.isArray(subjects)) {
            return APIResponse(res, 400, null, "Dữ liệu môn học không hợp lệ. Yêu cầu một mảng JSON.");
        }

        if (subjects.length === 0) {
            return APIResponse(res, 400, null, "Không có môn học nào được cung cấp để import.");
        }
...
export const downloadTemplateController = async (req, res) => {
    try {
        ...
        res.setHeader('Content-Disposition', 'attachment; filename=mon_hoc_mau.xlsx');
