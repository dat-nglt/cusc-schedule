...
export default function PreviewClassModal({
  ...
  const getRowStatus = (classItem) => {
    if (classItem.errors && classItem.errors.length > 0) {
      return 'error';
    }
    return 'valid';
  };
  ...
  const getErrorChip = (error) => {
    const errorMessages = {
      'duplicate_id': 'Mã lớp học trùng lặp',
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      const validData = validRows.map(row => {
        const { errors: _errors, rowIndex: _rowIndex, ...classData } = row;
        return classData;
      });
