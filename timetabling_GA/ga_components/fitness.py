...
from config import (
    ...
    PENALTY_CONSECUTIVE_HOURS_CLASS,
...
class FitnessCalculator:
    ...
    def calculate_fitness(self, chromosome):
        ...
        for gene in scheduled_events:
            ...
            # Bỏ qua các gene chưa được gán đầy đủ
            if not all([day, slot_id, lecturer_id, room_id, class_id, subject_id, lesson_type]):
                continue
            if "UNASSIGNED" in day or "UNASSIGNED" in slot_id or "UNASSIGNED" in lecturer_id or "UNASSIGNED" in room_id:
                continue
        ...
        for class_id, day_schedule in class_slots_per_day.items():
            if day_schedule:
